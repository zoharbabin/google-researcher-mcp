...
### Fixed
- **Faster MCP Startup**: PID lock acquisition now runs concurrently with instance initialization instead of blocking before it, and orphan kill wait reduced from 1500ms to 300ms — fixes first-connect timeout when Claude Code launches the server
